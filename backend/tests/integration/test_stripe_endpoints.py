...
- Webhook handler endpoint (including signature verification)
...
class TestWebhookEndpoint:
    ...
    def test_webhook_valid_signature_returns_200(self, client):
        """Test webhook with valid signature (mocked) returns 200 and processes event."""
        payload = json.dumps({
            "type": "payment_intent.succeeded",
            "data": {
                "object": {
                    ...
                    "currency": "try",
                    "status": "succeeded",
                    "metadata": {"user_id": "user_123", "product_id": "credits_100", "credits": 100}
                }
            }
        }).encode("utf-8")
        mock_event = {
        ...
        with patch("app.routers.stripe_router.stripe_service") as mock_stripe_svc:
            mock_stripe_svc.verify_webhook.return_value = mock_event
            response = client.post(
                "/api/stripe/webhook",
                data=payload,
                headers={
        ...
        # With valid (mocked) signature, endpoint should accept and return 200
