...
def _require_self_or_admin(current_user: dict, resource_user_id: str) -> None:
    """Raise 403 if current user is not the resource owner nor admin."""
    uid = current_user.get("user_id") or current_user.get("id")
    role = (current_user.get("app_metadata") or {}).get("role", "user")
    if str(uid) != str(resource_user_id) and role != "admin":
...
async def get_daily_quests(
    ...
    """
    Get or create daily quests for the user. Requires auth; user_id must match current user (or admin).
    """
...
async def check_quest_progress(
    ...
    """
    Manually trigger a check/update for a quest. Requires auth; quest must belong to current user (or admin).
    """
    # Ensure UUID
    try:
    ...
    _require_self_or_admin(current_user, quest.user_id)
...
async def get_user_achievements(
    ...
    """
    Get user achievements. Requires auth; user_id must match current user (or admin).
    """
    _require_self_or_admin(current_user, user_id)
    from app.services.achievement_service import achievement_service
